...
//========================== DriverProxyNBD ==================================//
// Ctor
    DriverProxyNBD::DriverProxyNBD(const std::string& dev_file, size_t storage_size) :
        m_socket_fd(0), m_dissconnect(false), m_id_counter(0)
    {
...
// nbd_thread
    void DriverProxyNBD::nbd_thread()
    {
        ...
        check_fail(sigprocmask(SIG_SETMASK, &sigset, NULL), "DriverProxyNBD: Sigprocmask Failed\n");

        // setting nbd sockets and flags
        check_fail(ioctl(m_nbd_fd, NBD_SET_SOCK, m_device_fd), "DriverProxyNBD: NBD_SET_SOCK failed\n"); 
                
        // optional:
        // flags |= NBD_FLAG_SEND_TRIM;
        // flags |= NBD_FLAG_SEND_FLUSH;
        check_fail(ioctl(m_nbd_fd, NBD_SET_FLAGS, flags), "DriverProxyNBD: NBD_SET_FLAGS Faild"); 
                
        // nbd do it - that's the main part! creating the conncection
        check_fail(ioctl(m_nbd_fd, NBD_DO_IT), "DriverProxyNBD: NBD_DO_IT Terminated with error"); 
                
        // cleaning up after disconnect
        check_fail(ioctl(m_nbd_fd, NBD_CLEAR_QUE), "DriverProxyNBD: NBD_CLEAR_QUEUE Failed");
        check_fail(ioctl(m_nbd_fd, NBD_CLEAR_SOCK), "DriverProxyNBD: NBD_CLEAR_SOCK Failed"); 
